...
// Provider type from frontend
interface OldProvider {
  id: string
  name: string
  type: string
  ryan_white_parts: string[]
  contact: {
    phone: string
    phone_24hr?: string
    email?: string
...
// Provider data from frontend
export const providers: OldProvider[] = [
  {
    ...
    description:
      'Comprehensive HIV care and support services in a welcoming, community-focused environment.',
    featured: true,
  },
  {
    id: 'rwj-aids-program',
  ...
  },
  {
    ...
    last_updated: '2025-01-15T10:00:00Z',
    description:
      'Comprehensive support services including housing assistance, support groups, and emergency financial assistance.',
    featured: true,
  },
  {
    id: 'zufall-health',
    ...
    last_updated: '2025-01-15T10:00:00Z',
    description:
      'Federally Qualified Health Center providing comprehensive medical care regardless of insurance status.',
  },
  {
    id: 'nbcc',
  ...
  },
  {
    ...
    last_updated: '2025-01-15T10:00:00Z',
    description:
      'Free legal assistance with benefits, housing, and HIV-related discrimination issues in Hunterdon County.',
  },
  {
    ...
    last_updated: '2025-01-15T10:00:00Z',
    description:
      'Free legal assistance with benefits, housing, and HIV-related discrimination issues in Somerset County.',
  },
  {
  ...
  },
  {
    ...
    last_updated: '2025-01-15T10:00:00Z',
    description:
      'Comprehensive sexual and reproductive health services including HIV testing and PrEP.',
  },
  {
    ...
    last_updated: '2025-01-15T10:00:00Z',
    description:
      'Local health department providing HIV testing and prevention services to township residents.',
  },
]

// Map old type to new type options
function mapProviderType(oldType: string): string {
  ...
  return typeMap[oldType] || 'Other'
}

// Convert hours object to text strings
...
// Extract service names from nested boolean structure
function extractServices(servicesObj: Record<string, boolean | string[]>): string[] {
...
export function transformProvider(oldProvider: OldProvider) {
  const firstLocation = oldProvider.locations[0]

  // Extract all services
  const medicalServices = extractServices(oldProvider.services.medical_care)
  const supportServices = extractServices(oldProvider.services.support_services)
  const preventionServices = extractServices(oldProvider.services.prevention)

  // Build eligibility requirements
  const eligibilityRequirements: string[] = []
  if (oldProvider.eligibility.income_requirements) {
    const incomeMap: Record<string, string> = {
      up_to_200_fpl: 'Income up to 200% of Federal Poverty Level',
      up_to_400_fpl: 'Income up to 400% of Federal Poverty Level',
    }
    eligibilityRequirements.push(
      incomeMap[oldProvider.eligibility.income_requirements] ||
        oldProvider.eligibility.income_requirements,
    )
  }

  oldProvider.eligibility.special_populations.forEach((pop) => {
    const popMap: Record<string, string> = {
      lgbtq: 'LGBTQ+ individuals',
      youth: 'Youth',
      women: 'Women',
      people_with_hiv: 'People living with HIV',
      adults: 'Adults',
      substance_use: 'People with substance use disorders',
      mental_health: 'People with mental health needs',
      uninsured: 'Uninsured individuals',
    ...
    eligibilityRequirements.push(popMap[pop] || pop)
  })
  ...
  return {
    ...
    type: mapProviderType(oldProvider.type),
    location: {
      address: firstLocation.address,
      city: firstLocation.city,
      state: 'NJ',
      zipCode: firstLocation.zip_code,
      county: firstLocation.county,
    },
    contact: {
      phone: oldProvider.contact.phone,
      phone24hr: oldProvider.contact.phone_24hr,
      email: oldProvider.contact.email,
      website: oldProvider.contact.website,
    },
    hours: {
      monday: formatHours(firstLocation.hours.monday),
      tuesday: formatHours(firstLocation.hours.tuesday),
      wednesday: formatHours(firstLocation.hours.wednesday),
      thursday: formatHours(firstLocation.hours.thursday),
      friday: formatHours(firstLocation.hours.friday),
      saturday: formatHours(firstLocation.hours.saturday),
      sunday: formatHours(firstLocation.hours.sunday),
    },
    services: {
      medical: medicalServices.map((service) => ({ service })),
      support: supportServices.map((service) => ({ service })),
      prevention: preventionServices.map((service) => ({ service })),
    },
    eligibility: eligibilityRequirements.map((requirement) => ({ requirement })),
    ryanWhite: oldProvider.ryan_white_parts.length > 0,
    ryanWhiteParts: oldProvider.ryan_white_parts,
    languages: oldProvider.languages.map((language) => ({
      language: language.charAt(0).toUpperCase() + language.slice(1),
    })),
    accessibility: firstLocation.accessibility.map((feature) => ({
      feature: feature
        .split('_')
        .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' '),
    })),
    insurance: oldProvider.eligibility.insurance_accepted.map((plan) => ({
      plan: plan.charAt(0).toUpperCase() + plan.slice(1),
    })),
    coordinates: {
      lat: firstLocation.coordinates.lat,
      lng: firstLocation.coordinates.lng,
    },
    status: 'active',
  }
}
  ...
  try {
    ...
    for (const oldProvider of providers) {
      try {
        ...
        console.log(`Importing: ${transformedProvider.name}`)
        ...
        successCount++
        console.log(`✅ ${transformedProvider.name} imported successfully\n`)
      } catch (error) {
        errorCount++
        console.error(`❌ Failed to import ${oldProvider.name}:`, error)
        console.error('\n')
      }
    }
